...
```
  ...
  To investigate the amount of time that dogs spend in the animal shelter before dying we used a Cox proportional hazards model. Cox proportional hazards is a semi parametric model used in survival analysis to explain how variables of interest affect the time until an event occurs. As the name 'survival analysis' suggests, this method is commonly used to investigate what factors affect time until death. This is done by comparing hazard rates using a hazard ratio. The hazard ratio is the ratio of hazard rates between groups, a hazard rate is the rate at which the outcome is happening. So, this type of model investigates how variables affect the rate at which outcomes happen.
...
We also found that the average time a pitbull spends in a shelter is longer than the average time a non-pitbull spends in a shelter. Pitbulls are spending more time in the animal shelter and therefore having more opportunities to be adopted and still have a higher proportion die in the animal shelter.
